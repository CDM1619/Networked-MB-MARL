# -*- coding: utf-8 -*-
"""
Created on Mon Feb 28 17:04:05 2022
...
class OnPolicyRunner:
    def __init__(self, logger, run_args, alg_args, agent, env_learn, env_test,env_args, **kwargs):
        ...
        if self.alg_name == 'IC3Net' and self.load_pretrained_model:
            self.agent.load_model(alg_args.pretrained_model)
    ...
    def test(self):
        ...
        for i in trange(1):
            ...
            while not (ep_len == length):
                ...
                if (self.env_name == 'Monaco' and self.alg_name == 'IC3Net') or (self.env_name == 'Grid' and self.alg_name == 'IC3Net'):
...
# --------------------------------------------------------------------------------------------------------------   
            ...
            if i+1 == self.n_test:                 
                ...
                for d in range(len(data)):
                    ...
                    avg_queue.append(data[d]['avg_queue'])
                    avg_wait_sec.append(data[d]['avg_wait_sec'])
                    
                    number_total_car.append(data[d]['number_total_car'])
                    number_departed_car.append(data[d]['number_departed_car'])
                    number_arrived_car.append(data[d]['number_arrived_car'])
                    avg_speed_mps.append(data[d]['avg_speed_mps'])
                ...
                plt.plot(time_sec, avg_queue)
                plt.xlabel('Steps',fontsize=15)
                plt.ylabel('avg_queue',fontsize=15)                   
                plt.show()



                plt.figure(figsize=(8,6))
                plt.plot(time_sec, avg_wait_sec)
                plt.xlabel('Steps',fontsize=15)
                plt.ylabel('avg_wait_sec',fontsize=15)                   
                plt.show()
                ...
                wb_1.save('./queue_'+str(self.alg_name)+'.xlsx') 
                ...
                wb_2.save('./wait_'+str(self.alg_name)+'.xlsx') 

                wb_3 = Workbook() #创建工作簿
                ws_3 = wb_3.active #激活工作表
                ws_3['A1'] = 'time_sec'
                ws_3['B1'] = 'number_total_car'                            
                for ii in range(len(time_sec)):                
                    ws_3.append([time_sec[ii],number_total_car[ii]])              
                wb_3.save('./number_total_car_'+str(self.alg_name)+'.xlsx') 

                wb_4 = Workbook() #创建工作簿
                ws_4 = wb_4.active #激活工作表
                ws_4['A1'] = 'time_sec'
                ws_4['B1'] = 'number_departed_car'                            
                for ii in range(len(time_sec)):                
                    ws_4.append([time_sec[ii],number_departed_car[ii]])              
                wb_4.save('./number_departed_car_'+str(self.alg_name)+'.xlsx') 

                wb_5 = Workbook() #创建工作簿
                ws_5 = wb_5.active #激活工作表
                ws_5['A1'] = 'time_sec'
                ws_5['B1'] = 'number_arrived_car'                            
                for ii in range(len(time_sec)):                
                    ws_5.append([time_sec[ii],number_arrived_car[ii]])              
                wb_5.save('./number_arrived_car_'+str(self.alg_name)+'.xlsx') 

                wb_6 = Workbook() #创建工作簿
                ws_6 = wb_6.active #激活工作表
                ws_6['A1'] = 'time_sec'
                ws_6['B1'] = 'avg_speed_mps'                            
                for ii in range(len(time_sec)):                
                    ws_6.append([time_sec[ii],avg_speed_mps[ii]])              
                wb_6.save('./avg_speed_mps_'+str(self.alg_name)+'.xlsx') 
