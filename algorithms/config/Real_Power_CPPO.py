...
def getArgs(radius_p, radius_v, radius_pi, env):
    ...
    agent_args = Config()
    agent_args.adj = np.ones((env.n_agents,env.n_agents))
    ...
    agent_args.use_reduced_v = True  # just use advantage rather than the reduced
    ...
    agent_args.embedding_sizes = [env.obs_size, 16, agent_args.hidden_state_dim]
    ...
    v_args.sizes = [-1, 128, 128, 1]    #最佳512
    agent_args.v_args = v_args

    pi_args = Config()
    pi_args.network = MLP
    pi_args.activation = torch.nn.ReLU
    pi_args.sizes = [-1, 128, 128, 16]    #最佳512
