...
from algorithms.envs.Large_net import Large_city_Env_2
from torch import distributed as dist



from algorithms.mbdppo.MB_DPPO_Large_city import OnPolicyRunner
...
def initEnv(input_args):
    if input_args.env == 'eight':
        ...
        env_fn_train, env_fn_test = Large_city_Env_2, Large_city_Env_2 
...
def parse_args():
    ...
    parser.add_argument('--env', type=str, required=False, default='Large_city', help="environment(eight/ring/catchup/slowdown/Grid/Monaco/PowerGrid/Real_Power/Pandemic/Large_city)")
    parser.add_argument('--algo', type=str, required=False, default='DPPO', help="algorithm(DMPO/IC3Net/CPPO/DPPO/IA2C) ")
    parser.add_argument('--device', type=str, required=False, default='cuda:2', help="device(cpu/cuda:0/cuda:1/...) ")
...
if run_args.profiling:
    ...
    OnPolicyRunner(logger = logger, run_args=run_args, alg_args=alg_args, agent=agent, env_learn=env_train, env_test = env_test,env_args=input_args).test(1)
